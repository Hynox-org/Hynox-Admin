...
export default function PrintLayout({ docTitle, docNumber, dateLabel, dateValue, company, billToName, billToAddress, items, showTax, taxPercent = 0, notes, className, }) {
    const subtotal = calcSubtotal(items);
    const tax = showTax ? calcTax(subtotal, taxPercent) : 0;
    const total = subtotal + tax;
    return (<div className={cn("bg-white text-foreground p-6 md:p-10 mx-auto w-full max-w-4xl", className)}>
      {/* Header */}
      <header className="flex items-start justify-between gap-6">
        <div className="flex items-center gap-4">
          <img src="/images/hynox-logo.jpg" alt="HYNOX logo" className="h-14 w-14 rounded-sm border border-gray-200 object-cover"/>
          <div>
            <h1 className="text-xl font-semibold">{company.name || "HYNOX"}</h1>
            {company.address && <p className="text-sm leading-6">{company.address}</p>}
            {(company.email || company.phone) && (<p className="text-sm">{[company.email, company.phone].filter(Boolean).join(" • ")}</p>)}
      ...
      <section className="mt-6">
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            const amt = qty * unit;
            return (<tr key={it.id} className="border-b last:border-b-0">
